...
public class DamengUtil {
    ...
    public static Connection initDameng(DatabaseConnectConfig config) {
        ...
        String ip;
        int port = 5236; // Default Dameng database port

        // Parse endpoint address (host:port)
        // Note: IPv6 format (e.g., [::1]:5236) is not supported, only IPv4 or hostname
        if (endpoint.contains(":")) {
            String[] parts = endpoint.split(":", 2); // Limit split to avoid IPv6 address issues
            ip = parts[0];
    ...
    public static String arrowTypeToJdbcType(ArrowType arrowType) {
        if (arrowType instanceof ArrowType.Utf8) {
            ...
            return "BIT"; // Dameng uses BIT type for boolean: 1=true, 0=false, NULL=null
        }
        else if (arrowType instanceof ArrowType.Date dateType) {
            return switch (dateType.getUnit()) {
                case DAY -> "DATE";
                case MILLISECOND -> "DATETIME(3)";  // Date(MILLISECOND) maps to DATETIME(3), distinct from Timestamp
            };
            ...
            int precision = switch (timeType.getUnit()) {
                case SECOND -> 0;      // TIME(0) - seconds precision
                case MILLISECOND -> 3; // TIME(3) - milliseconds precision
                case MICROSECOND -> 6; // TIME(6) - microseconds precision
                case NANOSECOND -> {
                    log.warn("Dameng database does not support nanosecond precision for TIME type. Using microseconds (6) instead.");
                    yield 6; // Fallback to microseconds
                }
            };
            ...
            return "BLOB"; // Large binary object (maximum length 100G-1 bytes)
        }
    ...
    public static ArrowType jdbcType2ArrowType(String jdbcType) {
        ...
        return switch (baseType) {
            ...
            case "TINYINT", "BYTE" -> new ArrowType.Int(8, true); // BYTE is an alias for TINYINT
            ...
            case "BOOLEAN", "BIT" -> ArrowType.Bool.INSTANCE; // Dameng uses BIT type for boolean: 1=true, 0=false, NULL=null

            case "DATE" -> new ArrowType.Date(DateUnit.DAY);

            case "VARBINARY", "RAW" -> ArrowType.Binary.INSTANCE; // RAW is an alias for VARBINARY
            ...
            for (String colName : allColumns) {
                ...
                if (fieldType instanceof ArrowType.Timestamp tsType) {
                    ...
                    if (tsType.getTimezone() != null && !tsType.getTimezone().isEmpty() && value instanceof Long) {
                        ...
                        value = dtf.format(zonedDateTime); // Convert to string format supported by Dameng
                    }
